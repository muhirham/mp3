...
class StockWhController extends Controller
{
    ...
    public function datatable(Request $r)
    {
        try {
            ...
            $statusFilter= trim((string) $r->input('status', ''));
            $from        = $r->input('from');
            $to          = $r->input('to');
            ...
            // ✅ FILTER status (baru jalan setelah base ada)
            if ($statusFilter !== '') {
                $base->where('rr.status', $statusFilter);
            }

            // ✅ FILTER tanggal (baru jalan setelah base ada)
            if ($from || $to) {
                if ($from && ! $to) $to = $from;
                if ($to && ! $from) $from = $to;
                ...
                try {
                    ...
                    $base->whereBetween('rr.created_at', [$fromC, $toC]);
                } catch (\Throwable $e) {
            ...
            // GROUP BY dokumen
            if ($hasCode) $base->groupBy('rr.code');
            else $base->groupBy('rr.id');

            // count group rows (bener)
            $recordsTotal = DB::query()->fromSub((clone $base)->selectRaw('1 as x'), 't')->count();

            // SEARCH
            if ($search !== '') {
                $like = '%' . $search . '%';
                $base->where(function ($q) use ($like, $hasCode, $noteCol) {
                    if ($hasCode) $q->where('rr.code', 'like', $like);
                    else $q->where('rr.id', 'like', $like);

                    $q->orWhere('p.name', 'like', $like)
                    ->orWhere('p.product_code', 'like', $like)
                    ->orWhere('s.name', 'like', $like);

                    if ($noteCol !== "''") $q->orWhereRaw($noteCol . ' LIKE ?', [$like]);
                });
            }

            $recordsFiltered = DB::query()->fromSub((clone $base)->selectRaw('1 as x'), 't')->count();
            ...
            $selects = [
                ...
                $noteCol === "''" ? DB::raw("'' as note") : DB::raw('MIN(' . $noteCol . ') as note'),
            ];
            $base->select($selects);

            // ORDER BY
            switch ($orderKey) {
                case 'qty_req':       $base->orderBy('qty_req', $orderDir); break;
                case 'qty_rcv':       $base->orderBy('qty_rcv', $orderDir); break;
                case 'product_name':  $base->orderBy('product_name', $orderDir); break;
                case 'supplier_name': $base->orderBy('supplier_name', $orderDir); break;
                case 'status':        $base->orderBy('status', $orderDir); break;
                case 'created_at':    $base->orderBy('created_at', $orderDir); break;
                case 'note':          $base->orderBy('note', $orderDir); break;
                case 'code':
                default:              $base->orderBy('code', 'desc'); break;
            }
            ...
            $rows = $base->skip($start)->take($length)->get()
                ->map(function ($r, $idx) use ($start) {
                    ...
                    if ($status === 'approved')      $badge = '<span class="badge bg-label-info">REVIEW</span>';
                    elseif ($status === 'ordered')   $badge = '<span class="badge bg-label-secondary">ORDERED</span>';
                    elseif ($status === 'received')  $badge = '<span class="badge bg-label-primary">RECEIVED</span>';
                    elseif ($status === 'cancelled') $badge = '<span class="badge bg-label-dark">CANCELLED</span>';
                    else                             $badge = '<span class="badge bg-label-warning">PENDING</span>';
                    ...
                    if ($itemCnt > 1) {
                        ...
                        $subLine = $subLine !== '' ? $subLine . ' · ' . $extraText : $extraText;
                    }
                    ...
                    $actions .= '<button type="button" class="btn btn-icon btn-sm btn-outline-secondary js-detail"
                        title="Detail"
                        data-id="' . (int)$r->id . '"
                        data-code="' . e($r->code) . '">
                    ...
                    if ($canReceive) {
                        $actions .= '<button type="button" class="btn btn-icon btn-sm btn-outline-primary js-receive"
                            title="Receive"
                            data-id="' . (int)$r->id . '"
                            data-code="' . e($r->code) . '"
                    ...
                    return [
                        ...
                        'status'     => $badge,
                        'created_at' => $r->created_at ? Carbon::parse($r->created_at)->format('Y-m-d') : '-',
                        'note'       => e($r->note ?? ''),
                        'actions'    => $actions,
                    ];
                });
            ...
            \Log::error('restocks.datatable: ' . $e->getMessage(), ['trace' => $e->getTraceAsString()]);
