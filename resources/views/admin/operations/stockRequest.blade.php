...
@section('content')
    ...
    <script>
        ...
        async function fetchList() {
            ...
            if (!res.ok) {
                ...
                Swal.fire({
                    ...
                    html: '<pre style="text-align:left">' + t.replace(/[<>&]/g, s => ({
                        '<': '&lt;',
                        '>': '&gt;',
                        '&': '&amp;'
                    } [s])) + '</pre>',
      ...
      <tr>
        <td>
          ${r.status === 'pending' ? `<input type="checkbox" class="chk-row" value="${r.id}">` : ''}
        </td>
        <td>${r.id}</td>
    ...
    `);
      ...
      <a class="page-link" href="#" data-page="${p}">${txt}</a></li>`);
        ...
        window.addEventListener('reverb:stock-request-updated', (e) => {
            console.log('Real-time Update Received: Admin Pusat (via Event Bus)');
