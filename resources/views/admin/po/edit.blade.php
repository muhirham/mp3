  ...
  @php
    ...
    // ini dipakai buat @disabled dan @unless di form item
    $poLocked       = $baseLocked || $approvalLocked;

    // boleh submit approval hanya kalau masih draft / rejected & grand_total ada
    $canSubmitApproval = !$poLocked
  ...
  <div class="container-xxl flex-grow-1 container-p-y">
    ...
    <div class="d-flex justify-content-end mt-2 gap-2">
      ...
      {{-- Cancel cuma boleh kalau belum di-approval flow --}}
  ...
  @if($canApproveProc || $canApproveCeo)
    <div class="card mt-3">
      ...
      <div class="card-body">
        @if($canApproveProc)
          <div class="mb-3">
            <form method="POST" action="{{ route('po.approve.proc', $po->id) }}" class="d-inline">
              @csrf
            ...
            <form method="POST" action="{{ route('po.reject.proc', $po->id) }}" class="d-inline">
              @csrf
        ...
        @if($canApproveCeo)
          <hr>
          <div class="mb-3">
            <form method="POST" action="{{ route('po.approve.ceo', $po->id) }}" class="d-inline">
              @csrf
              <button type="submit" class="btn btn-success me-2">
                Approve CEO
              </button>
            ...
            <form method="POST" action="{{ route('po.reject.ceo', $po->id) }}" class="d-inline">
              @csrf
              <div class="input-group">
                <input type="text"
                      name="reason"
