...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h4 class="fw-bold py-1 mb-0">Daily Deposits / <span class="text-primary text-opacity-75">New Deposit</span>
                </h4>
        ...
        <div class="card mb-4 shadow-sm">
            <div class="card-body">
                <form id="filterForm" action="{{ route('warehouse.settlements.create') }}" method="GET"
                    ...
                    <div class="col-md-{{ auth()->user()->hasRole(['superadmin', 'admin'])? '3': '5' }}">
                        ...
                        <input type="date" name="date_start" class="form-control border-light bg-light"
                            value="{{ request('date_start') }}">
                    </div>
                    <div class="col-md-{{ auth()->user()->hasRole(['superadmin', 'admin'])? '3': '5' }}">
                        <label class="form-label text-muted small fw-bold text-uppercase">Date End</label>
                        <input type="date" name="date_end" class="form-control border-light bg-light"
            ...
            <div class="card mb-4 bg-primary text-white shadow-none">
                <div class="card-body">
                    <div class="row align-items-center">
                        <div class="col-md-8">
                            <h5 class="text-white mb-1"><i class="bx bx-layer me-2"></i> Grand Total Unsettled (Filtered)
                            </h5>
                            <p class="mb-0 opacity-75">There are <span id="grandCount">{{ $grandCount }}</span>
                                transactions matching your filters.</p>
                            <div class="d-flex mt-3">
                                <div class="me-4 border-end pe-4">
                                    <small class="d-block opacity-75 text-uppercase"
                                        style="font-size: 0.65rem; letter-spacing: 1px;">Total Cash</small>
                                    <h4 class="text-white mb-0" id="grandCashDisplay">Rp
                                        {{ number_format($grandCash, 0, ',', '.') }}</h4>
                                </div>
                                <div>
                                    <small class="d-block opacity-75 text-uppercase"
                                        style="font-size: 0.65rem; letter-spacing: 1px;">Total Transfer</small>
                                    <h4 class="text-white mb-0" id="grandTFDisplay">Rp
                                        {{ number_format($grandTf, 0, ',', '.') }}</h4>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-4 text-md-end mt-4 mt-md-0">
                            <button type="button" class="btn btn-lg btn-light text-primary shadow-sm" id="btnBulkSettle">
                                <i class="bx bx-check-double me-1"></i> Deposit Selected (Bulk)
                            </button>
                        </div>
                    </div>
                </div>
                        ...
                        @forelse($unsettled as $row)
                            <tr>
                                <td class="text-center py-3">
                                    <input type="checkbox" class="form-check-input hdo-checkbox"
                                        value="{{ $row->id }}" data-cash="{{ $row->cash_amount }}"
                                        data-tf="{{ $row->transfer_amount }}">
                                </td>
                                <td class="py-3">
                                ...
                                <td class="py-3">
                                    ...
                                    <span
                                        class="badge bg-label-secondary small">{{ $row->is_direct_sale ? 'Direct Sale (SI)' : 'Handover (HDO)' }}</span>
                                </td>
                                <td class="py-3">
                                    <span
                                        class="badge bg-label-info">{{ $row->warehouse->warehouse_name ?? $row->warehouse->name }}</span>
                                </td>
                                <td class="py-3">
                                ...
                                <td class="py-3 text-end text-info fw-bold">
                                    Rp {{ number_format($row->transfer_amount, 0, ',', '.') }}
                                </td>
                                <td class="py-3 text-center">
                                    <button type="button" class="btn btn-sm btn-primary shadow-sm btn-settle-single"
                                        data-id="{{ $row->id }}"
                                        data-formatted="{{ $row->code }} - {{ $row->sales->name }}"
                                        data-cash="{{ number_format($row->cash_amount, 0, ',', '.') }}"
                                        data-tf="{{ number_format($row->transfer_amount, 0, ',', '.') }}">
    ...
    <div class="modal fade" id="settleModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <form id="settleForm" action="{{ route('warehouse.settlements.store') }}" method="POST"
                enctype="multipart/form-data">
                @csrf
                <div id="modal_id_container"></div>
                        ...
                        <div class="row mb-4 text-center">
                            <div class="col-6 border-end">
                                <small class="text-muted d-block mb-1 text-uppercase"
                                    style="font-size: 0.7rem; letter-spacing: 1px;">Total Cash</small>
                                <h4 class="text-success mb-0 fw-bold" id="modal_cash">Rp 0</h4>
                            </div>
                            <div class="col-6">
                                <small class="text-muted d-block mb-1 text-uppercase"
                                    style="font-size: 0.7rem; letter-spacing: 1px;">Total Transfer</small>
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function initSettleButtons() {
                ...
                document.querySelectorAll('.btn-settle-single').forEach(btn => {
                    btn.onclick = function() {
                        ...
                        const container = document.getElementById('modal_id_container');
                    ...
                    btnBulkSettle.onclick = function() {
                        ...
                        if (checked.length === 0) {
                            Swal.fire({
                                ...
                                text: 'Please select at least one transaction to deposit.',
                        ...
                        checked.forEach(cb => {
                            totalCash += parseInt(cb.dataset.cash);
                            totalTf += parseInt(cb.dataset.tf);
                            idsHtml += `<input type="hidden" name="ids[]" value="${cb.value}">`;
                        });

                        const container = document.getElementById('modal_id_container');
                        container.innerHTML = idsHtml;

                        document.querySelector('#modalTitle span').innerText =
                            `${checked.length} Transactions (Bulk)`;
                        document.getElementById('modal_cash').innerText = 'Rp ' + new Intl.NumberFormat('id-ID')
                            .format(totalCash);
                        document.getElementById('modal_tf').innerText = 'Rp ' + new Intl.NumberFormat('id-ID')
                            .format(totalTf);
                ...
                if (checkAll) {
                    checkAll.onchange = function() {
                        document.querySelectorAll('.hdo-checkbox').forEach(cb => {
            ...
            function applyFilters(page = 1) {
                ...
                fetch(url, {
                    ...
                    .then(data => {
                        if (data.success) {
                            // Update Totals
                            grandCashDisplay.innerText = 'Rp ' + new Intl.NumberFormat('id-ID').format(data
                                .grand_cash);
                            grandTFDisplay.innerText = 'Rp ' + new Intl.NumberFormat('id-ID').format(data
                                .grand_tf);
                            ...
                            if (data.unsettled.length === 0) {
                                tableBody.innerHTML =
                                    '<tr><td colspan="8" class="text-center py-5"><div class="py-4"><i class="bx bx-check-circle display-1 text-light"></i><h5 class="mt-3 text-muted">All caught up! No pending transactions.</h5></div></td></tr>';
                            } else {
                                data.unsettled.forEach(row => {
                                    const dateRaw = new Date(row.handover_date);
                                    const date = dateRaw.toLocaleDateString('id-ID', {
                                        day: '2-digit',
                                        month: 'short',
                                        year: 'numeric'
                                    });
                                    const cash = new Intl.NumberFormat('id-ID').format(row.cash_amount);
                                    const tf = new Intl.NumberFormat('id-ID').format(row
                                        .transfer_amount);
                                    const whName = row.warehouse ? (row.warehouse.warehouse_name || row
                                        .warehouse.name) : '-';
                                    const salesName = row.sales ? row.sales.name : '-';
                                    const typeBadge = row.is_direct_sale ?
                                        '<span class="badge bg-label-secondary small">Direct Sale (SI)</span>' :
                                        '<span class="badge bg-label-secondary small">Handover (HDO)</span>';

                                    tableBody.innerHTML += `
                            <tr>
                                <td class="text-center py-3">
                                    <input type="checkbox" class="form-check-input hdo-checkbox" value="${row.id}" data-cash="${row.cash_amount}" data-tf="${row.transfer_amount}">
                                </td>
                                <td class="py-3">
                                ...
                                <td class="py-3 text-center">
                                    <button type="button" class="btn btn-sm btn-primary shadow-sm btn-settle-single" 
                                        data-id="${row.id}"
                                        data-formatted="${row.code} - ${salesName}"
                                        data-cash="${cash}"
                                        data-tf="${tf}">
                                        <i class="bx bx-send me-1"></i> Settle
                                    </button>
            ...
            // 🔥 HANDLE MODAL SUBMIT (AJAX - NO REFRESH COK)
                    ...
                    fetch(this.action, {
                        ...
                        .then(data => {
                            if (data.success) {
                                modal.hide();
                                this.reset();
                                document.getElementById('modal_id_container').innerHTML = '';
            ...
            if (typeof Echo !== 'undefined') {
                ...
                // Listen for closed handovers (new pending data)
