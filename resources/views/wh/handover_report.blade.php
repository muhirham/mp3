...
@php
    ...
    $pageTitle = $isSales && ! $isAdminLike && ! $isWarehouse
        ? 'Daily Report Handover Saya'
        : ($isWarehouse ? 'Sales Reports (Admin Warehouse)' : 'Sales Reports');
          ...
          @else
            <select name="warehouse_id" class="form-select">
              ...
              @foreach($warehouses as $w)
                @php
                  $whLabel = $w->warehouse_name ?? $w->name ?? $w->warehouse_code ?? ('Warehouse #'.$w->id);
                @endphp
                <option value="{{ $w->id }}" @selected($warehouseId == $w->id)>{{ $whLabel }}</option>
              @endforeach
            </select>
          @endif
        </div>
      ...
      <div class="table-responsive">
        <table class="table table-sm table-hover align-middle">
          ...
          <tbody id="handoverRows">
            @forelse(($rows ?? []) as $r)
              @if($view==='sales')
                <tr>
                  ...
                  <td class="text-end">
                    <button type="button" class="btn btn-sm btn-outline-primary btn-drill-sales" data-sales-id="{{ $r['sales_id'] }}">
                      Lihat
                    </button>
                  </td>
                </tr>
              @elseif($view==='daily')
                <tr>
                  ...
                  <td class="text-end">{{ $r['amount_setor'] }}</td>
                  <td class="text-end">
                    <button type="button" class="btn btn-sm btn-outline-primary btn-drill-day" data-date="{{ $r['date'] }}">
                      Lihat
                    </button>
                  </td>
                </tr>
            ...
            @empty
              <tr>
                <td colspan="10" class="text-center text-muted">Belum ada data pada periode ini.</td>
              </tr>
            @endforelse
          </tbody>
...
(function () {
  ...
  function renderRows(view, rows) {
    if (!rows || !rows.length) {
      rowsTbody.innerHTML = `<tr><td colspan="10" class="text-center text-muted">Belum ada data pada periode ini.</td></tr>`;
      return;
    }
  ...
  async function reloadList() {
    ...
    try {
      const res = await fetch(listUrl + '?' + params.toString(), { headers: { 'X-Requested-With': 'XMLHttpRequest' } });
      ...
      console.error(err);
      Swal.fire({ icon:'error', title:'Gagal', text:'Gagal memuat data report.' });
    }
  }
  ...
  filterForm.querySelectorAll(autoSelectors.join(',')).forEach(el => el.addEventListener('change', reloadList));
  filterForm.addEventListener('submit', (e) => { e.preventDefault(); reloadList(); });
  ...
  rowsTbody.addEventListener('click', async (e) => {
    ...
    if (btnDetail) {
      ...
      try {
        ...
        console.error(err);
        Swal.fire({ icon:'error', title:'Gagal', text:'Gagal memuat detail handover.' });
      }
      return;
    }
