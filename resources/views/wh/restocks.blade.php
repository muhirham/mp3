                ...
                <div class="row g-2 align-items-end">
                    ...
                    <div class="col-12 col-md-3 ms-md-auto">
                        ...
                        @elseif($isWarehouseUser)
                            <input class="form-control form-control-sm" value="{{ $me->warehouse?->warehouse_name ?? '-' }}"
                                disabled>
...
.modal-backdrop {
    z-index: 1050 !important;
        ...
        .restock-subtable thead th {
            font-size: .72rem;
            text-transform: uppercase;
            letter-spacing: .04em;
        }

        .restock-subtable tbody td {
            font-size: .82rem;
        }
...
@push('scripts')
    ...
    <script>
        ...
        `);
            ...
            // ===== AUTO BALANCE GOOD & DAMAGED =====
            $(document).on('input', '#tblReceiveItems input[type="number"]', function() {

                const $row = $(this).closest('tr');

                const qtyRemain = parseInt($row.find('td:eq(5)').text()) || 0;

                const $goodInput = $row.find('input[name*="[qty_good]"]');
                const $damageInput = $row.find('input[name*="[qty_damaged]"]');

                let good = parseInt($goodInput.val()) || 0;
                let damage = parseInt($damageInput.val()) || 0;

                // Prevent negative
                if (good < 0) good = 0;
                if (damage < 0) damage = 0;

                // Kalau yg diubah damaged
                if ($(this).is($damageInput)) {

                    if (damage > qtyRemain) damage = qtyRemain;

                    good = qtyRemain - damage;

                    $damageInput.val(damage);
                    $goodInput.val(good);
                }

                // Kalau yg diubah good
                if ($(this).is($goodInput)) {

                    if (good > qtyRemain) good = qtyRemain;

                    damage = qtyRemain - good;
...
$('#formReceive').on('submit', function(e){
    ...
    const modalEl = document.getElementById('mdlReceive');
    ...
    setTimeout(function(){
        ...
        // bersihin backdrop sisa kalau ada
        ...
        Swal.fire({
            ...
            $.ajax({
                ...
                success: function(res){
                    ...
                    $('#tblRestocks').DataTable().ajax.reload(null,false);
                }
            });
...
// ===== SUBMIT REQUEST RESTOCK =====
$('#formAdd').on('submit', function(e){
    e.preventDefault();

    const form = this;
    const modalEl = document.getElementById('mdlAdd');
    const modal   = bootstrap.Modal.getInstance(modalEl);

    // ✅ LANGSUNG TUTUP MODAL
    modal.hide();

    // bersihin backdrop paksa (biar gak nempel)
    setTimeout(function(){
        $('body').removeClass('modal-open');
        $('.modal-backdrop').remove();
